...
struct pocs_allocator
{
  ...
  template <class U>
  pocs_allocator(pocs_allocator<U> const &rhs) : x_{rhs.x_}
  {
  }
...
template <class T>
struct is_nothrow_member_swappable
{
...
namespace
{
  ...
  {
    template <class T>
    void operator()(T &x1, T &x2) const { x1.swap(x2); }
  } member_fn_swap;

  struct
  {
    template <class T>
    void operator()(T &x1, T &x2) const
    {
  ...
  void swap_tests(X *, F swapper, GF gen_factory, test::random_generator rg)
  {
    ...
    auto vals1 = make_random_values(1024 * 8, [&]
                                    { return gen(rg); });
    auto vals2 = make_random_values(1024 * 4, [&]
                                    { return gen(rg); });
