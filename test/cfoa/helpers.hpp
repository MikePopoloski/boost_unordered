...
private:
  template <class>
  friend struct stateful_allocator2;
...
template <class Tag>
std::atomic<std::uint32_t> basic_raii<Tag>::default_constructor(0);
template <class Tag>
std::atomic<std::uint32_t> basic_raii<Tag>::copy_constructor(0);
template <class Tag>
std::atomic<std::uint32_t> basic_raii<Tag>::move_constructor(0);
template <class Tag>
std::atomic<std::uint32_t> basic_raii<Tag>::destructor(0);
template <class Tag>
std::atomic<std::uint32_t> basic_raii<Tag>::copy_assignment(0);
template <class Tag>
std::atomic<std::uint32_t> basic_raii<Tag>::move_assignment(0);
...
void thread_runner(std::vector<T> &values, F f)
{
...
template <class T>
void shuffle_values(std::vector<T> &v)
{
...
public:
  ...
  fancy_allocator() {}
  template <class Y>
  fancy_allocator(fancy_allocator<Y> const &) {}
...
{
  template <>
  struct pointer_traits<void_ptr>
  {
    template <class U>
    struct rebind_to
    {
