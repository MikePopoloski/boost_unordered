...
struct poca_allocator : fancy_allocator<T>
{
  ...
  template <class U>
  poca_allocator(
      poca_allocator<U, POCCA, POCMA> const &rhs) : super{rhs}, x_{rhs.x_}
  {
  }
...
namespace
{
  ...
  void copy_assign(X *, GF gen_factory, test::random_generator rg)
  {
    ...
    auto values = make_random_values(1024 * 16, [&]
                                     { return gen(rg); });
      ...
      X *, GF gen_factory, test::random_generator rg)
  {
    ...
    auto gen = gen_factory.template get<X>();
    auto values = make_random_values(4, [&]
                                     { return gen(rg); });
  ...
  void insert_and_assign(X *, GF gen_factory, test::random_generator rg)
  {
    ...
    auto v1 = make_random_values(1024 * 16, [&]
                                 { return gen(rg); });
    auto v2 = v1;
    shuffle_values(v2);
